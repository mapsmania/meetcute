 const map = new maplibregl.Map({
      ...
      zoom: 2,
      attributionControl: false // turn off default placement
});

// Create control with custom attribution
const attributionControl = new maplibregl.AttributionControl({
  compact: false,
  customAttribution: [
    '<a href="https://googlemapsmania.blogspot.com/2025/08/the-map-to-your-heart.html" target="_blank">About</a>'
  ]
});

// Add to your custom container
document.getElementById('map-attribution').appendChild(attributionControl.onAdd(map));

    const storyContainer = document.getElementById('story-container');
    const storyPlaceholder = document.getElementById('story-placeholder');

  const romanceRules = {
 "start": "$title % $meet % $moment % $obstacle % $resolution",
...
// use custom geocoder to fetch nearest town to user click
...
map.on('click', (e) => {
  ...
  const result = geocoder.reverse(lat, lon);

  if (result && result.name) {
    const story = generateRomanceStory(result.name);
    ...
    storyPlaceholder.textContent = "No town found.";
  }
});
